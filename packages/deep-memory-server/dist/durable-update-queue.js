...
function stableTranscriptHash(messages) {
...
function sanitizeFailedTask(task, file) {
    return {
        file,
        key: task.key,
...
export class DurableUpdateQueue {
    ...
    pendingFilesByKey = new Map(); // best-effort "latest"
    idleWaiters = [];
    constructor(params) {
        this.log = params.log;
    ...
    async init() {
        ...
        for (const name of inflight) {
            ...
            try {
                const task = await readJson(from);
        ...
        for (const name of pending) {
            ...
            try {
                ...
                if (!prev) {
                    ...
                    const prevTask = await readJson(prev);
...
//# sourceMappingURL=durable-update-queue.js.map