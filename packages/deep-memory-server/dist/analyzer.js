...
function detectUserIntentScore(text) {
...
function topKFrequency(tokens, k) {
    const map = new Map();
...
function extractIsoTimestampHint(text) {
...
export function extractHintsFromText(input) {
...
function detectMemoryKind(text) {
    const t = text.toLowerCase();
...
function guessSubject(entities, topics) {
...
function guessExpiresAt(kind, text, now) {
    if (kind !== "ephemeral") {
        return undefined;
    }
    const t = text.toLowerCase();
...
export class SessionAnalyzer {
    analyze(params) {
        ...
        for (const item of params.messages) {
            const record = item && typeof item === "object" && "role" in item
                ? item
        ...
        const topics = topicTerms.map((t) => ({
            name: t.term,
            frequency: t.count,
            importance: Math.min(1, t.count / 10),
        }));
        // Entities: heuristic extraction of "named" terms.
        const entityCandidates = topKFrequency(tokens, 30).filter((t) => /[A-Za-z]/.test(t.term) || /[\u4e00-\u9fff]/.test(t.term));
        const entities = entityCandidates.slice(0, 10).map((e) => ({
            name: e.term,
