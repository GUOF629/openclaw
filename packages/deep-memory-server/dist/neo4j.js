...
export class Neo4jStore {
    ...
    async getSessionIngestMeta(params) {
        const session = this.driver.session();
        try {
            const res = await session.run(`MATCH (s:Session {id: $id})
         RETURN s.last_transcript_hash AS hash, s.last_message_count AS count`, { id: this.sessionNodeId(params.namespace, params.sessionId) });
    ...
    async setSessionIngestMeta(params) {
        const session = this.driver.session();
        try {
            await session.run(`MATCH (s:Session {id: $id})
    ...
    async upsertTopic(params) {
        const session = this.driver.session();
        try {
            await session.run(`MERGE (t:Topic {id: $id})
    ...
    async upsertEntity(params) {
        const session = this.driver.session();
        try {
            await session.run(`MERGE (e:Entity {id: $id})
    ...
    async upsertEvent(params) {
        const session = this.driver.session();
        try {
            await session.run(`MERGE (e:Event {id: $id})
    ...
    async linkSessionEvent(params) {
        const session = this.driver.session();
        try {
            await session.run(`MATCH (s:Session {id: $sid})
         MATCH (e:Event {id: $eid})
         MERGE (s)-[:HAS_EVENT]->(e)`, { sid: this.sessionNodeId(params.namespace, params.sessionId), eid: params.eventId });
        }
        finally {
            await session.close();
        }
    ...
    async linkEventTopic(params) {
        const session = this.driver.session();
        try {
            await session.run(`MATCH (e:Event {id: $eid})
         MERGE (t:Topic {id: $tid})
         ON CREATE SET t.name = $name, t.frequency = 0, t.importance = 0, t.namespace = $ns
         MERGE (e)-[:ABOUT_TOPIC]->(t)`, { eid: params.eventId, tid: this.topicNodeId(params.namespace, params.topicName), name: params.topicName, ns: params.namespace });
        }
        finally {
            await session.close();
        }
    ...
    async linkSessionTopic(params) {
        ...
        try {
         ...
         MERGE (t:Topic {id: $tid})
         ON CREATE SET t.name = $name, t.frequency = 0, t.importance = 0, t.namespace = $ns
         MERGE (s)-[:CONTAINS]->(t)`, { sid: this.sessionNodeId(params.namespace, params.sessionId), tid: this.topicNodeId(params.namespace, params.topicName), name: params.topicName, ns: params.namespace });
        }
        finally {
            await session.close();
        }
    ...
    async linkTopicEntity(params) {
        const session = this.driver.session();
        try {
            await session.run(`MATCH (t:Topic {id: $tid})
    ...
    async linkMemoryTopic(params) {
        const session = this.driver.session();
        try {
            await session.run(`MATCH (m:Memory {id: $mid})
         MERGE (t:Topic {id: $tid})
         ON CREATE SET t.name = $name, t.frequency = 0, t.importance = 0, t.namespace = $ns
         MERGE (m)-[:ABOUT_TOPIC]->(t)`, { mid: params.memoryId, tid: this.topicNodeId(params.namespace, params.topicName), name: params.topicName, ns: params.namespace });
        }
        finally {
            await session.close();
        }
    ...
    async linkMemoryRelated(params) {
        const session = this.driver.session();
        try {
            await session.run(`MATCH (a:Memory {id: $a})
         MATCH (b:Memory {id: $b})
         MERGE (a)-[r:RELATED_TO]->(b)
         SET r.score = greatest(coalesce(r.score, 0), $score),
             r.updated_at = datetime()`, { a: params.fromMemoryId, b: params.toMemoryId, score: params.score });
        }
        finally {
            await session.close();
        }
    ...
    async queryRelatedMemories(params) {
        const session = this.driver.session();
        try {
            const res = await session.run(`WITH $entities AS entities, $topics AS topics, $prefix AS prefix
    ...
    async deleteMemoriesByIds(params) {
        ...
        const session = this.driver.session();
        try {
            const res = await session.run(`MATCH (m:Memory)
         WHERE m.id IN $ids
         DETACH DELETE m
    ...
    async deleteMemoriesBySession(params) {
        const session = this.driver.session();
        try {
            const res = await session.run(`MATCH (m:Memory)-[:FROM_SESSION]->(s:Session {id: $sid})
         DETACH DELETE m
         RETURN count(*) AS deleted`, { sid: this.sessionNodeId(params.namespace, params.sessionId) });
            const row = res.records[0];
            const deleted = row ? Number(row.get("deleted") ?? 0) : 0;
            return Number.isFinite(deleted) ? deleted : 0;
        }
