...
describe("DeepMemoryUpdater", () => {
  it("filters sensitive + low-importance drafts and processes important ones", async () => {
    ...
    const qdrantUpserts: Array<{ id: string; payload: any }> = [];
    let searchCalls = 0;
    const qdrant = {
      search: async ({ limit }: { limit: number }) => {
        ...
        if (searchCalls === 1) return [{ id: "mem_existing", score: 0.95, payload: undefined }];
        if (searchCalls === 2) return [{ id: "mem_other", score: 0.2, payload: undefined }];
      ...
      upsertMemory: async ({ id, payload }: { id: string; payload: any }) => {
